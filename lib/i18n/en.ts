...
export const t = {
  ...
  pwaTest: {
    title: "PWA Health",
    devNote: "Service worker is not registered in development mode.",
    sections: {
    ...
    gps: {
      ...
      elapsedLabel: "Time to fix",
      errorDenied: "Permission denied",
      errorTimeout: "Timed out",
      errorUnknown: "Could not get location",
      accuracyMetres: (m: number) => `±${Math.round(m)} m`,
      elapsedMs: (ms: number) => `${ms} ms`,
    },
    cacheVersion: (key: string) => `Cache: ${key}`,
