...
export interface TimelineHandle {
  jumpTo(isoTimestamp: string, highlightId?: string): void;
}
...
export const Timeline = forwardRef<TimelineHandle, TimelineProps>(
  function Timeline(
    { entries, onPickTime, onCreateDuration, onDayChange }: TimelineProps,
    ref,
  ) {
    const now = useNow();
    ...
    useImperativeHandle(
      ref,
      () => ({
        jumpTo(isoTimestamp: string, highlightId?: string) {
          if (now === null) return;
          const targetMs = new Date(isoTimestamp).getTime();
          const today = startOfDay(new Date(now));
          const target = startOfDay(new Date(targetMs));
          const offset = differenceInCalendarDays(target, today);
          pendingScrollMs.current = targetMs;
          setDayOffset(offset);
    ...
    const points = useMemo(
      () =>
        entries.filter(
          (e) =>
            e.entryType === "point_label" || e.entryType === "point_number",
        ),
      [entries],
    );
    ...
    function handleLanePointerMove(e: PointerEvent<HTMLDivElement>) {
      ...
      if (!anchor || anchor.pointerType === "touch" || !onCreateDuration)
        return;
      const py = e.clientY - e.currentTarget.getBoundingClientRect().top;
    ...
    // Cancel highlight timeout on unmount.
    useEffect(() => {
      return () => {
        if (highlightTimeoutRef.current !== null)
          clearTimeout(highlightTimeoutRef.current);
    ...
    useEffect(() => {
      ...
      let targetY: number;
      if (pendingScrollMs.current !== null) {
        targetY = yPos(pendingScrollMs.current) - VIEWPORT_PX / 3;
        pendingScrollMs.current = null;
      } else if (showingToday && nowY !== null) {
        ...
        >
          ...
          >
            ...
            <div
              ...
              onPointerMove={
                onCreateDuration ? handleLanePointerMove : undefined
              }
              onPointerUp={
            ...
            >
              ...
              {visibleOrphans.map((entry) => (
                ...
                >
                  <span
                    className={cn(
                      "shrink-0 rounded-sm bg-amber-500 transition-all duration-200",
                      entry._id === highlightedId
                        ? "size-4 animate-pulse border-2 border-background ring-2 ring-amber-500"
                        : "size-2",
                    )}
                  />
              ...
              {visiblePoints.map((entry) => (
                ...
                >
                  <span
                    className={cn(
                      "shrink-0 rounded-full bg-primary transition-all duration-200",
                      entry._id === highlightedId
                        ? "size-4 animate-pulse border-2 border-background ring-2 ring-primary"
                        : "size-2",
                    )}
                  />
