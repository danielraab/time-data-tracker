...
managed by `npx @next/codemod agents-md` — **do not edit between the NEXT-AGENTS-MD
markers**; add new content above this block instead.

## Session continuity — read these first

If you are picking up this project in a new session, start by reading:

- [`docs/plan.md`](docs/plan.md) — the implementation plan, including a status
  snapshot at the top and the outline of remaining phases (auth, backend sync,
  sharing). Update the snapshot as phases land.
- [`docs/insights/`](docs/insights/) — saved Claude Code `/insights` reports
  with usage patterns, friction analysis, and suggested next steps for this
  project.

Also run `git log --oneline -20` and `git status` to see how the working tree
has diverged from the last snapshot.

## What this project is

TiDaTra (Time & Data Tracker) is an **offline-first PWA** for recording time series. See
`README.md` for the full feature spec. Key constraints that shape every decision:

- **Local-first**: the app must be fully usable with no account and no network. Data is
  written to PouchDB in the browser first; the server is a sync target, not the source of
  truth.
- **Sync on login**: when a user signs in, local PouchDB data syncs bidirectionally with
  a per-user CouchDB database. Design data models to be replication- and
  conflict-friendly (CouchDB revisions, deterministic doc `_id`s).
- **The UI is in English** for now. All user-facing strings live in `lib/i18n/en.ts`; additional locales may be added later, so never inline copy in components.

## Tech stack

- Next.js 16 (App Router) + React 19 + TypeScript — App Router only, no Pages Router.
- Tailwind CSS v4 (`@tailwindcss/postcss`) + shadcn/ui for components.
- better-auth for auth: OAuth providers, Authentik (OIDC), magic-link email.
- PouchDB (client) ⇄ CouchDB (server) for data and sync.
- pnpm for package management. Always use `pnpm`, never `npm`/`yarn`.
- Use the newest stable version of any dependency added.

## Domain model (target)

- **ZeitSerie** (time series): `title`, `description`, `tags[]`, owner, shares.
- **Eintrag** (entry): belongs to a series. Always has a timestamp. Optional GPS, optional
  label. Entry types: point-in-time label, point-in-time number, duration start (label =
  the duration's name), duration end.
- **Open duration**: a duration with only a start or only an end — must be visually
  flagged and surfaced on the dashboard.
- **Tag**: free-form, created on the fly, many-to-many with series.
- **Share**: a series can be shared with another user/email as read-only or editable.

## Conventions

- Server Components by default; add `"use client"` only when a component needs browser
  APIs, state, or PouchDB. Note PouchDB and geolocation are client-only.
- Keep all PouchDB/CouchDB access behind a dedicated client-side data layer; do not call
  it directly from UI components.
- Environment variables: see `.env.example`. Anything browser-exposed must be prefixed
  `NEXT_PUBLIC_`. Never commit real secrets.
- The dev container (`.devcontainer/`) provides CouchDB on `localhost:5984`.
...
search and read the relevant file in `./.next-docs` (index below).
